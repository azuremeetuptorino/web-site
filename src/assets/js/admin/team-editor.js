 ...
 * editor-core.js, condiviso con gli altri due editor. Qui ci sono solo i campi.
 */

const PLACEHOLDER_AVATAR = '/assets/img/placeholder-avatar.svg';

export const teamEditor = createEditorCore({
    endpoint: '/api/team',
    ...
    lists: {
        members: {
            ...
            /** Un membro nuovo parte da Staff: il gruppo meno impegnativo da correggere. */
            ...
            collect(row, index, { value, field }) {
                const member = {
                    ...
                    // La posizione nella lista e l'unica fonte di verita.
                    order: (index + 1) * 10,
                    active: field(row, 'active').checked
                };

                // Riga del link lasciata vuota = questo membro non ha un link.
                // A meta la lascia passare: e il server a dire quale pezzo manca.
    ...
    fill(data, { setList }) {
        setList('members', data.members ?? []);
    },

    collect({ readList }) {
        return { version: 1, members: readList('members') };
    }
});
